...
class ChatGroupOptions {
  ...
  /// 设置群组属性。
  ///
  /// Param [style] 群组的类型。详见 {ChatGroupStyle}。
  ///
  /// Param [maxCount] 群组的最大成员数，默认为 200。
  ///
  /// Param [inviteNeedConfirm] 邀请用户进群是否需要对方同意。收到邀请是否自动入群取决于两个设置：创建群组时设置 inviteNeedConfirm 以及通过 [ChatOptions.setAutoAcceptGroupInvitation] 确定是否自动接受加群邀请。
  /// 具体使用如下：
  ...
  /// [ChatOptions.setAutoAcceptGroupInvitation] 为 SDK 级别操作，设置为 'true' 时，受邀人收到入群邀请后，SDK 在内部调用同意入群的 API，
  /// 自动接受邀请入群；
