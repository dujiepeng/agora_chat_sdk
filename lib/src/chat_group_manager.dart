...
class ChatGroupManager {
  ...
  ///
  /// **Return** 包含用于下次获取数据的 cursor 以及群组列表。返回的结果中，当 `ChatCursorResult.getCursor()` 为空字符串 ("") 时，表示没有更多数据。
  ///
  /// **Throws**  如果有异常会在此抛出，包括错误码和错误信息，详见 [ChatError]。
  /// ~end
  ...
  /// 可通过设置 [ChatClient.addMultiDeviceEventHandler] 监听相关事件，事件回调函数为 [ChatMultiDeviceEventHandler.onGroupEvent]，第一个参数为事件，创建群组事件为 [ChatMultiDevicesEvent.GROUP_CREATE]。
  ///
  ...
  ///
  /// Param [options] 群组的其他选项。请参见 [ChatGroupOptions]。
  /// 群组的其他选项。
  /// - 群最大成员数，默认值为 200；
  ...
  ///
  /// **Return** 分页获取结果 [ChatCursorResult]，包含用于下次获取数据的 cursor 以及群组成员列表。返回的结果中，当 [ChatCursorResult.cursor] 为空字符串 ("") 时，表示没有更多数据。
  ///
  /// **Throws**  如果有异常会在此抛出，包括错误码和错误信息，详见 [ChatError]。
  /// ~end
